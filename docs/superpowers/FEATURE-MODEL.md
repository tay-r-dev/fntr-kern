...
`capBallSide`. `capAngle` negates, and so do the serif's `axisAngle` and `axisTilt` — the tilt
for its own reason rather than by analogy: a mirror takes the serif frame to `(−M(axis),
M(depth))`, because the axis is re-oriented to the new left while depth still points into the
stroke, so a tilt of θ on the old frame is a tilt of −θ on that one.
...
`axisMode` is `perpendicular`, `horizontal`, `vertical`, `absolute` or `tilt`. `absolute` takes
`axisAngle` and `tilt` takes `axisTilt`. The code holds the axis at least
`MIN_AXIS_TANGENT_SEPARATION_DEG` (15°) off the tangent, on the side it arrived from. An axis
that approaches the tangent makes the terminal degenerate, and the wings start to lie along the
stroke instead of across it. Clamped across the tangent instead, an axis arriving a fraction inside
the band leaves 30° from where it came in, which is a jump.

**`tilt` is the perpendicular turned off the rib by a stated angle**, and it is a mode so that the
plain perpendicular stays the default. Its whole difference from `absolute` is that it is stated
against the rib rather than against the glyph, so it follows a leaning stroke and it interpolates
between masters whose stems lean differently. The rotation is applied AFTER the axis is oriented
toward the contour's left, so one number reads the same at both ends of a stroke: positive tilt
turns the axis toward the stroke on the left, so the left wing climbs the stem and the right one
...
— at the wing-swallowed snap, where the tip's own line stops crossing the wall, and where the
corner ray runs parallel to it. None of the three belongs to the tilt; each is reachable through an
absolute axis at the same effective angle. The panel offers ±40, which is the range that moves
smoothly, and the writer clamps nothing, because the shape does not stop there.
...
Six fields sit at terminal level and both halves share them: `axisMode`, `axisAngle`, `axisTilt`,
...
lopsided. That is a handle configuration a designer does not want to work with,
...
- **The whole press runs on a copy and only the points that ended up somewhere
  else are written back.** Both steps write handles the construction already
  wrote, and every one of those writes would be a recorded change.
- **It moves the outer handles**, which the constructions never do. The repair is
  the nearest answer and the nearest answer moves all four handle lengths.
...
- **One pass per press, so the press is not always a fixed point.** The
  construction re-states its ratio from the balanced drawing on the next press.
  It converges: `N^1.json` settles on the first press, `I^1.json` on the second
  after 2.6 more units.
- Measured on a joint whose segments arrive at a tension gap of 0.600: the
  construction alone leaves 0.554 and the pass leaves 0.092. On `N^1.json` the
  worst gap over the glyph goes 0.194 to 0.045.
