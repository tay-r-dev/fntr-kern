# Current input and preview contract — approved 2026-09-09

This section supersedes the historical two-input and `%member%!` decisions below.

- Remove Pair input. Glyph accepts comma-separated plain names, characters, and
  `@class` names. Old `/name` input remains accepted; pointer shortcuts emit plain names.
- Empty Glyph imposes no glyph restriction. One glyph shows its relationships on
  either side (narrowed by Side). Multiple glyphs show all ordered combinations
  between them, including repeated-glyph pairs. An unknown name shows an inline error.
- Ctrl-click replaces Glyph; Ctrl+Shift-click toggles a glyph in its list.
- Shift-click in the scene toggles the adjacent pairs touching that **occurrence**.
  Selections combine as alternatives and intersect with the Glyph filter. A removable
  Preview pairs chip makes this extra restriction visible. Changing Glyph or the
  phrase clears it. Pair previews generated by table selection do not clear it.
- In `hello`, Shift-click e means he and el only. It never invents hh, ee, or ll.
  Shift-click the first l does include el and ll, because ll is on screen there.
- Filtering does not tick or apply rows. Existing table highlighting and Apply/Reset
  selection stay separate. Highlighted summaries preview concrete pairs within the
  input scope, but applying them still writes the full displayed class address.
- Naming a member does not expose individual rows. Show individual is the exposure
  control. Class-to-class and class-to-unique summaries show `@class(member)` for
  focused members. Saved exceptions remain separate.
- Unicode types filter partners of a single focus glyph. With multiple or no focus
  glyphs, both sides must belong to selected types. Classes qualify through matching
  concrete member pairs. Use the font's actual Unicode assignments, not name guesses;
  an unencoded alternate is Non-Unicode, even if its name resembles an encoded glyph.
- Pair and phrase preview share per-pair suggestion ribbons and numbers above them.
  The kerning tool shows current values below each ribbon, including zero. Suggestion
...
user's request. The input test file was updated to the new contract for later use.

1. Try empty input, h, then h/e/l; check both directions, self-pairs, Side, and clear.
2. In `hello`, Ctrl+Shift-click h/e/l, then Shift-click e: only he/el relationships.
   Toggle it off; Shift-click either l and compare the actual adjacent pairs.
3. Enter a grouped member, toggle Show individual, and inspect class/unique summaries
   and saved exceptions. Confirm Apply still targets the displayed rule address.
4. Filter an encoded glyph with an arbitrary name, an accented letter, a combining
   mark, a number, and an unencoded alternate by Unicode types.
5. Preview several pairs with proposals; check every line's ribbon/number. Toggle
   suggestions on/off, including a pair with existing kerning, and engage the kerning
   tool to check numbers below the ribbons.
6. In Font mode, scroll, change source, edit class colors/membership, and check both
   class edges persist while editor-status bars remain absent.
