...
crosses the foot line in one place and stands `flankSlope × wingSlope` further along by the time the
bracket has climbed the wing. Something has to absorb that distance. There are exactly two places it
can go, and **the designer chose the bracket** (2026-08-07, on `_external/c.json`: a 17 degree stem,
a 20 unit wing slope, 12 units to absorb).
...
stroke. The visible cost is that one bracket reaches further sideways than the other, by the wing
slope times the lean. At wing slope 0 the two are identical, because then the bracket does not climb.
...
serif lets go, so a flank-built terminal leaves a visible step. But a point taken from the cut is a
function of the edge's shape, and therefore of everything that shapes the edge. So **a
curvature pin started moving two on-curves along the stroke**, which is not what that gizmo does
anywhere else in the editor. See §9.

The cost is explicit and accepted. Where the stroke wall has curved off the flank by the height the
serif grabs at, the code bends the wall back to meet the terminal. On `_external/g.json` as drawn
we measured 2.6 units. At the very bottom of the curvature range it reaches about 32 units, where
the wall is a chord nowhere near the flank. The lever for that is the serif's reach, not the pin.
...
A trim makes the **emitted** segment shorter than the segment the generator solved. Anything that
measures the emitted one measures a different curve from the one a pin governs. So the split
publishes the segment it cut from, on the inserted point's provenance, as `constructionSegment`.
`generatedSegmentConstructionPoints` resolves it for every reader.

This is not serif-specific plumbing. Round caps trim too, and they trim only a sliver. The serif is
what made the error visible, at 83 units on a real glyph. See §9.
