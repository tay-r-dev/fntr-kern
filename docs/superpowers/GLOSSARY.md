...
distribution say. It reads the link flag and never writes it, so releasing A leaves the point
linked as the designer left it, with a new distribution.
...
where the stroke meets the foot line, shifted by half the difference between the wings, and the
**cup balance** slides it from there out onto either tip.
...
one preset stays correct on an upright foot and a slanted one alike. Every master holds one
named **Default**, per case, and its own beside it. Right-click on the preset dropdown resets a
serif terminal to its master's Default.

**Default** (serif) — the plain slab foot, and the shape a terminal gets the moment you pick serif: wing,
