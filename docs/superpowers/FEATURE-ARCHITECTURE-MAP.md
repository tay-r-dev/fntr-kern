...
| `fontra-core/src/bulb-geometry.js` | — | **NEW** — rib-anchored ball, glyph-axis apexes and harmonious entry via the shared V-slide and nearest harmonizer. The generator owns the inner cut/neck and construction provenance. `test-bulb-geometry.js` checks axis handles, entry curvature and parameter sweeps. |
...
| `fontra-core/src/skeleton-width-rate.js`   | 60    | **NEW** — the width between on-curves: the rate an on-curve owns (`jointWidthRate`), the eased width at t (`easedWidth`), and the edge's angle off the skeleton (`edgeTiltAngle`). The generator's `segmentEdgeRates` applies it per contour. |
...
2. **Letterspacer-to-skeleton coupling — closed.** Both writers move the skeleton with the
   sidebearing now. Letterspacer's Apply always did (`applySpacingToLayerGlyph` in
   `panel-letterspacer.js`); the selection-info margin path did not, so typing a left margin on a
   skeleton glyph shifted the outline and left the centerline behind, and the metrics-keys Update
   and Update-all passes inherited it. `setLeftMarginOnLayer` in `panel-selection-info.js` is the
   one place that math lives and it carries the skeleton through `editSkeleton` (rail R-C).
