...
| `fontra-core/src/bulb-geometry.js` | — | **NEW** — rib-apex ball frame and arcs. The generator owns the restored inner-wall crossing and neck; the outer wall stays intact. `test-bulb-geometry.js` checks the fixed forward radius, outer wall, and parameter sweeps. |
