...
describe("skeleton-generator drop caps", () => {
  ...
  // The ball region in contour order: from the ease-in point, the last on-curve
  // still on the outer edge before the ball, to the landing back on the inner
  // edge. On this horizontal stroke the outer edge is y=80 and the inner y=160.
  function ballOnCurves(capFields) {
    return generateFromSkeleton(horizontalDrop(capFields)).contours[0].points.filter(
      (p) => !p.type && p.x > 241
    );
  }

  it("puts the ball's on-curves on its glyph-axis extremes", () => {
    // Default ratio 1.25 on width 80: lateral radius 50, so the ball's top sits
    // on y=80, its bottom on y=180, and its tip on the terminal plane x=440.
    const ball = ballOnCurves({});
    expect(ball.filter((p) => Math.abs(p.x - 440) <= 1)).to.have.length(1);
    expect(ball.filter((p) => Math.abs(p.y - 180) <= 1)).to.have.length(1);
    expect(ball.filter((p) => Math.abs(p.y - 160) <= 1)).to.have.length.of.at.least(1);
  });

  // The ball's points sit on its glyph-axis extremes, so the number of them is
  // the number of extremes the sweep covers, and that changes with the sweep.
  // What must NOT change is which extremes are chosen for a given sweep: the
  // three variations of the underscore straddle the meeting with the wall, and
  // an earlier rule picked a different three either side of it.
  it("chooses the same extremes either side of the meeting", () => {
    const ballPoints = (handleDy) =>
      generateFromSkeleton({
        version: 1,
        nextId: 10,
        contours: [
          {
            ...
            points: [
              ...
              { id: 3, x: 325, y: 227 + handleDy, type: "cubic" },
              { id: 4, x: 259, y: 324, type: "cubic" },
              { id: 5, x: 149, y: 250, type: null, smooth: false },
            ],
          },
        ],
        generated: [],
      }).contours[0].points.filter((point) => !point.type).length;
    const counts = [-4, -2, 0, 2, 4].map(ballPoints);
...
describe("skeleton-generator: the edge turns with the width", () => {
  ...
  it("turns a terminal's handle by 10 degrees at most", () => {
    ...
    off = ((off + 540) % 360) - 180;
    // The handle is rounded to the grid, which adds up to a few degrees on a
    // short one.
    expect(Math.abs(off)).to.be.at.most(12);
  });
